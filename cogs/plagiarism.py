...
def plagiarism(text: str) -> dict:
    ...
    res = requests.post(plag_checker_url, headers=headers, cookies=cookies, data=request_data)
...
class Plagiarism(commands.Cog):
    ...
    @commands.command(
        ...
        help='check attached txt file for plagiarism'
    )
    ...
    @commands.has_role('Instructor')
    @commands.command(
        name='grade_for_plagiarism',
        help=''
    )
    async def grade_for_plagiarism(self, ctx, member_name: str, assignment_name: str, max_plagiarism_percent: str):
        ...
        if max_plagiarism_percent > 1:
            await ctx.send('max_plagiarism_percent is a decimal. Ex: 0.5 represents 50%')
            return
        ...
        if (100 - float(res["percent"])) >= (max_plagiarism_percent*100):  # greater-than or equal to % plagiarized
            grade_exists = db.query(
                '''SELECT COUNT(grade)
                FROM grades
            ...
            if grade_exists:
                ...
                await ctx.author.send('Assignment was plagiarized, grade newly inputted to zero')
                    
        else:
            # No plagiarized
            await ctx.author.send('Assignment not was plagiarized. If a grade exists, it was unchanged')
